...
from pygls.plugins.references import pygls_references
from pygls._utils import PY2
...
def test_references(tmp_workspace):  # pylint: disable=redefined-outer-name
    ...
    refs = pygls_references(doc1, position)
    ...
    no_def_refs = pygls_references(doc1, position, exclude_declaration=True)
...
def test_references_builtin(tmp_workspace):  # pylint: disable=redefined-outer-name
    ...
    refs = pygls_references(doc2, position)
    assert len(refs) >= 1
