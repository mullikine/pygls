...
class GlossaryLanguageServer(MethodDispatcher):
    ...
    def capabilities(self):
        server_capabilities = {
            ...
            'executeCommandProvider': {
                'commands': flatten(self._hook('pygls_commands'))
            },
            'hoverProvider': True,
            ...
            'experimental': merge(self._hook('pygls_experimental_capabilities'))
        }
    ...
    def m_initialize(self, processId=None, rootUri=None, rootPath=None, initializationOptions=None, **_kwargs):
        ...
        self._dispatchers = self._hook('pygls_dispatchers')
        self._hook('pygls_initialize')
    ...
    def m_initialized(self, **_kwargs):
        self._hook('pygls_initialized')

    def code_actions(self, doc_uri, range, context):
        return flatten(self._hook('pygls_code_actions', doc_uri, range=range, context=context))

    def code_lens(self, doc_uri):
        return flatten(self._hook('pygls_code_lens', doc_uri))

    def completions(self, doc_uri, position):
        completions = self._hook('pygls_completions', doc_uri, position=position)
        return {
            'isIncomplete': False,
    ...
    def definitions(self, doc_uri, position):
        return flatten(self._hook('pygls_definitions', doc_uri, position=position))

    def document_symbols(self, doc_uri):
        return flatten(self._hook('pygls_document_symbols', doc_uri))

    def execute_command(self, command, arguments):
        return self._hook('pygls_execute_command', command=command, arguments=arguments)

    def format_document(self, doc_uri):
        return self._hook('pygls_format_document', doc_uri)

    def format_range(self, doc_uri, range):
        return self._hook('pygls_format_range', doc_uri, range=range)

    def highlight(self, doc_uri, position):
        return flatten(self._hook('pygls_document_highlight', doc_uri, position=position)) or None

    def hover(self, doc_uri, position):
        return self._hook('pygls_hover', doc_uri, position=position) or {'contents': ''}
    ...
    def lint(self, doc_uri, is_saved):
        ...
        if doc_uri in workspace.documents:
            workspace.publish_diagnostics(
                doc_uri,
                flatten(self._hook('pygls_lint', doc_uri, is_saved=is_saved))
            )

    def references(self, doc_uri, position, exclude_declaration):
        return flatten(self._hook(
            'pygls_references', doc_uri, position=position,
            exclude_declaration=exclude_declaration
        ))

    def rename(self, doc_uri, position, new_name):
        return self._hook('pygls_rename', doc_uri, position=position, new_name=new_name)

    def signature_help(self, doc_uri, position):
        return self._hook('pygls_signature_help', doc_uri, position=position)

    def folding(self, doc_uri):
        return self._hook('pygls_folding_range', doc_uri)
    ...
    def m_text_document__did_open(self, textDocument=None, **_kwargs):
        ...
        self._hook('pygls_document_did_open', textDocument['uri'])
    ...
    def m_workspace__did_change_configuration(self, settings=None):
        self.config.update((settings or {}).get('pygls', {}))
