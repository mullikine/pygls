...
from pygls import _utils, hookspecs, uris, PYLS
    ...
    def __init__(self, root_uri, init_opts, process_id, capabilities):
        ...
        for name, plugin in self._pm.list_name_plugin():
            if plugin is not None:
                log.info("Loaded pygls plugin %s from %s", name, plugin)

        for plugin_conf in self._pm.hook.pygls_settings(config=self):
